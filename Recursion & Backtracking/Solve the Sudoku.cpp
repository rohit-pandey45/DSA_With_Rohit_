class Solution {
  ...
  bool isValid(vector<vector<int>>& mat, int row, int col, int n ){
      for(int i = 0; i<9; i++){
  ...
  bool solve(vector<vector<int>>& mat){
      ...
      for(int i = 0; i<mat.size(); i++){
          for(int j = 0; j<mat[0].size(); j++){
              ...
              if(mat[i][j] == 0){       //Empty Space Found
                  for(int k = 1; k<=9; k++){        //Passing the numbers that can be filled
                      if(isValid(mat, i, j, k)){   //Checking if the number satisfies the position
                          ...
                          if(solve(mat) == true){   //Recursive Call for the next empty space in the space row
                            ...
                            mat[i][j] = 0;   //Change it back to 0 and again search for the number that can be placed there
                        }
                      }
